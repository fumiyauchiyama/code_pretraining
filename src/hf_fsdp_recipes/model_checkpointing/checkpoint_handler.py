...
def save_model_and_optimizer_sharded(
        ...
        rank, 
        dist_checkpoint_folder: str,
        model_name: str="checkpoint-shaded",
        optim=None, 
        verbose=True
        ):
...
def save_model_checkpoint(
    ...
    model_save_name: str="checkpoint",
    epoch=1,
    verbose: bool=True
):
    ...
    if not checkpoint_type == StateDictType.FULL_STATE_DICT:
        print(f" unable to handle checkpoint type {checkpoint_type}, aborting")
    ...
    if rank == 0:
        ...
        # create save path
        save_dir = Path.cwd() / checkpoint_folder
        save_name = model_save_name + "-" + str(epoch)
        save_full_path = str(save_dir) + "/" + save_name

        # save model
        #torch.save(cpu_state, save_full_path)
        model.save_pretrained(
            save_directory=save_full_path,
...
def load_model_checkpoint(
    ...
    full_state_dict_model_path = (
        Path.cwd() / checkpoint_folder / checkpoint_model_filename
    )
    # is it present...
...
def save_optimizer_checkpoint(
        ...
        rank: int, 
        checkpoint_folder: str,
        optimizer_name: str="checkpoint-optim",
        epoch: int=1,
        verbose: bool=True
        ):
    ...
    if rank == 0:
        ...
        opt_save_name = (
            optimizer_name + "-" + checkpoint_folder + "-" + str(epoch) + ".pt"
        )
...
def load_optimizer_checkpoint(
    ...
    opt_file_path = Path.cwd() / checkpoint_folder / optimizer_checkpoint_file
...
def load_distributed_model_checkpoint(
        model, 
        rank, 
        checkpoint_type: StateDictType,
        dist_checkpoint_root_folder: str,
...
def save_distributed_model_checkpoint(
        model, 
        checkpoint_type: StateDictType,
        dist_checkpoint_root_folder: str,
        dist_checkpoint_folder: str,
