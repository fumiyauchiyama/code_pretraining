...
mpirun -np $NUM_GPUS \
  ...
  model=gpt2_2048 \
  dataset=thestack_python_ablation_cf_r \
  train=train_shared train.batch_size=24 train.test_batch_size=24 \
  train.output_dir="/home/uchiyama.fumiya/ucllm/ucllm/program/outputs/models/exp11c/thestack_python_ablation_cf_r" \
  exec.use_fp16=false preprocess.shuffle.seed=2
