...
mpirun -np $NUM_GPUS \
  ...
  train=train_shared train.batch_size=32 train.test_batch_size=32 train.epochs=5 \
  preprocess.preprocess=["shuffle","avoid_short_sample","select","train_test_split","pack_texts"]
