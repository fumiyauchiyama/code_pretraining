...
#SBATCH --nodes=1
#SBATCH --job-name=outputs-llama-fineweb-1
#SBATCH --output=outputs/llama-fineweb-1.out
...
mpirun -np $num_gpus \
  ...
  model=llama_1_5b \
  dataset=exp5c_fineweb_tinyllama \
  preprocess=preprocess_exp5c \
  train=train_shared train.batch_size=24 train.test_batch_size=24 \
  train.output_dir="/storage9/fumiyau/ucllm/program/outputs/models/exp5c/exp5c_fineweb_llama" \
  exec.use_fp16=false
